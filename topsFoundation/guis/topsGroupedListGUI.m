classdef topsGroupedListGUI < topsGUI
    ...
    methods
        ...
        function showDetailsForCurrentItem(self)
            ...
            % shallow look at the item
            if ischar(self.currentMnemonic)
                col = self.getColorForString(self.currentMnemonic);
            else
                col = [0 0 0];
            end
            ...
            if ~isempty(item) && any(strcmp(methods(item), 'gui'))
                self.itemDetailGrid.newControlAtRowAndColumn( ...
                    1, [width-1 width], ...
                    'Style', 'pushbutton', ...
                    'String', 'gui', ...
                    'HitTest', 'on', ...
                    'Callback', @(obj, event)item.gui, ...
                    'HorizontalAlignment', 'left', ...
            ...
            elseif iscell(item)
                for ii = 1:length(item)
                    ...
                    if isa(item{ii}, 'function_handle') && ~isempty(which(func2str(item{ii})))
                        self.itemDetailGrid.newControlAtRowAndColumn( ...
                            1, [1 width-1], ...
                            'Style', 'text', ...
                            'String', stringifyValue(item), ...
                        ...
                        self.itemDetailGrid.newControlAtRowAndColumn( ...
                            ...
                            'String', 'open', ...
                            'HitTest', 'on', ...
                            'Callback', @(obj,event)open(func2str(item{ii})), ...
                            'HorizontalAlignment', 'left', ...
                            'BackgroundColor', self.lightColor, ...
                            'ForegroundColor', [0 0 0]);
                    else
